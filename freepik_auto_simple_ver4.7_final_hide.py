...
백그라운드 파일 업로드 지원 (filechooser 이벤트 사용)
"""
import asyncio
...
class FreepikGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Freepik 자동화 프로그램 - ver4.7_final_hide")
        # 창 크기 줄이기
    ...
    def create_widgets(self):
        # 제목
        title_label = tk.Label(self.root, text="Freepik 자동화 프로그램", font=("Arial", 14, "bold"))
        title_label.pack(pady=5)
        
        # 지정모드 설정 프레임 (자동모드 숨김)
        manual_frame = ttk.LabelFrame(self.root, text="파일 선택", padding=8)
        manual_frame.pack(fill="x", padx=15, pady=5)
        
        manual_inner = tk.Frame(manual_frame)
        manual_inner.pack()
        
        tk.Button(manual_inner, text="모델 파일 선택", command=self.select_model_files, width=18).grid(row=0, column=0, padx=5, pady=3)
        self.model_files_label = tk.Label(manual_inner, text="선택된 파일: 없음", fg="gray", font=("Arial", 9))
        self.model_files_label.grid(row=0, column=1, padx=5, pady=3, sticky="w")
        
        tk.Button(manual_inner, text="클로즈 파일 선택", command=self.select_clothes_files, width=18).grid(row=1, column=0, padx=5, pady=3)
        self.clothes_files_label = tk.Label(manual_inner, text="선택된 파일: 없음", fg="gray", font=("Arial", 9))
        self.clothes_files_label.grid(row=1, column=1, padx=5, pady=3, sticky="w")
        
        # 진행 상황 표시 (프롬프트 설정 위쪽)
        self.progress_frame = tk.Frame(self.root)
        self.progress_frame.pack(fill="x", padx=15, pady=5)
        
        self.progress_label = tk.Label(self.progress_frame, text="대기 중...", font=("Arial", 10, "bold"), fg="gray")
        self.progress_label.pack()
        
        # 프롬프트 입력 프레임
        prompt_frame = ttk.LabelFrame(self.root, text="프롬프트 설정", padding=8)
        prompt_frame.pack(fill="both", expand=True, padx=15, pady=5)
        
        # 프롬프트 리스트 및 입력 영역 (기본 3개만 보이고 스크롤)
        prompt_list_frame = tk.Frame(prompt_frame)
        prompt_list_frame.pack(fill="both", expand=True, pady=3)
        
        # 스크롤 가능한 프레임 (높이 제한)
        canvas = tk.Canvas(prompt_list_frame, height=120)
        ...
        for i in range(1, 11):
            ...
            ttk.Radiobutton(row_frame, text=str(i), variable=self.selected_prompt_num, 
                           value=i, command=lambda n=i: self.on_prompt_selected(n)).pack(side="left", padx=3)
            ...
            # 텍스트 입력창 (Entry 대신 Text 사용, 크기 줄이기)
            text_widget = tk.Text(row_frame, height=1, wrap=tk.WORD, width=35, font=("Arial", 9))
            text_widget.pack(side="left", fill="x", expand=True, padx=3)
            ...
            status_label = tk.Label(row_frame, text="", fg="green", width=8, font=("Arial", 8))
        ...
        tk.Button(save_button_frame, text="프롬프트 저장", command=self.save_all_prompts, 
                 bg="#2196F3", fg="white", font=("Arial", 9, "bold"), width=12, height=1).pack()
        
        # 초기 선택
        self.on_prompt_selected(1)
        
        # 다운로드 폴더 설정 프레임
        download_frame = ttk.LabelFrame(self.root, text="다운로드 폴더", padding=5)
        download_frame.pack(fill="x", padx=15, pady=3)
        
        download_inner = tk.Frame(download_frame)
        download_inner.pack()
        
        tk.Label(download_inner, text="폴더:", font=("Arial", 9)).grid(row=0, column=0, padx=3, pady=2, sticky="w")
        display_download_path = str(self.download_dir) if len(str(self.download_dir)) <= 35 else f"...{str(self.download_dir)[-32:]}"
        self.download_dir_label = tk.Label(download_inner, text=display_download_path, fg="gray", wraplength=300, font=("Arial", 8))
        self.download_dir_label.grid(row=0, column=1, padx=3, pady=2, sticky="w")
        tk.Button(download_inner, text="선택", command=self.select_download_dir, width=8, font=("Arial", 8)).grid(row=0, column=2, padx=3, pady=2)
        ...
        self.start_button = tk.Button(start_frame, text="작업 시작", command=self.start_automation, 
                                      bg="#4CAF50", fg="white", font=("Arial", 11, "bold"), width=12, height=1)
        self.start_button.pack(side="left", padx=3)
        
        self.stop_button = tk.Button(start_frame, text="작업 중지", command=self.stop_automation,
                                     bg="#f44336", fg="white", font=("Arial", 11, "bold"), width=12, height=1, state="disabled")
        self.stop_button.pack(side="left", padx=3)
        
        # 상태 표시
        self.status_label = tk.Label(self.root, text="대기 중...", font=("Arial", 9), fg="gray")
        self.status_label.pack(pady=2)
        
        # 로그 창 (아래쪽)
        log_frame = ttk.LabelFrame(self.root, text="작업 로그", padding=5)
        ...
        self.log_text = tk.Text(log_text_frame, height=8, wrap=tk.WORD, font=("Consolas", 8), state="disabled")
    ...
    def start_automation(self):
        ...
        self.settings = {
            ...
            "clothes_files": self.clothes_files,
            "download_dir": str(self.download_dir)  # 다운로드 폴더
        }
        
        # 총 루프 개수 계산
        self.total_loops = len(self.model_files) * len(self.clothes_files)
        self.current_loop = 0
        ...
        self.status_label.config(text="작업 실행 중...", fg="blue")
        self.progress_label.config(text=f"진행: 0/{self.total_loops}", fg="blue")
        self.log_text.config(state="normal")
        ...
        def run_automation():
            try:
                ...
                loop.run_until_complete(connect_and_work(self.settings, self))
                loop.close()
    ...
    def stop_automation(self):
        ...
        self.is_running = False
        self.status_label.config(text="중지 요청 중...", fg="orange")
        messagebox.showinfo("중지", "작업 중지가 요청되었습니다. 현재 작업이 완료되면 중지됩니다.")
    ...
    def on_automation_finished(self):
        ...
        self.progress_label.config(text=f"완료: {self.current_loop}/{self.total_loops}", fg="green")
        self.log("=" * 50)
        self.log("작업 완료!")


async def connect_and_work(settings, gui_instance=None):
    # 다운로드 폴더 설정
    ...
    log_msg("=" * 50)
    log_msg("Freepik 자동화 - 기존 크롬 연결")
    log_msg("=" * 50)
    log_msg("\n중요: 크롬을 디버깅 모드로 실행해야 합니다!")
    log_msg("\n크롬 디버깅 모드 실행 방법:")
    log_msg("1. 모든 크롬 창을 닫으세요")
    log_msg("2. start_chrome_debug.bat 파일을 실행하세요")
    log_msg("   (또는 수동으로 명령 실행)")
    log_msg("3. 크롬이 열리면 Freepik에 로그인하세요")
    log_msg("4. 이 프로그램을 실행하세요")
    log_msg("\n자동으로 진행합니다...")
    # GUI에서 실행 버튼을 눌렀으므로 자동으로 진행
    import time
    ...
    async with async_playwright() as p:
        try:
            ...
            log_msg(f"현재 페이지: {page.url}")
            
            # Freepik 페이지로 이동 (필요시)
            if "freepik.com" not in page.url:
                log_msg("Freepik 페이지로 이동 중...")
            ...
            async def upload_file(file_name, upload_button_selector, folder_path=None):
                """파일 업로드 함수 - 백그라운드 처리 (filechooser 이벤트 사용)"""
                # 전체 파일 경로 생성
                if folder_path:
                ...
                # 모달 창 내 파일 선택 버튼 찾기 및 클릭 (filechooser 이벤트 사용)
                print("모달 창 내 파일 선택 버튼 찾는 중...")
                modal_btn = None
                ...
                if not modal_btn:
                    ...
                    print("기존 방식(pyautogui)으로 폴백합니다...")
                    # 기존 pyautogui 방식으로 폴백
                    time.sleep(3.0)
                    ...
                    if not full_file_path.exists():
                        ...
                        raise FileNotFoundError(f"파일을 찾을 수 없습니다: {full_file_path}")
                    
                    # filechooser 이벤트를 기다리면서 버튼 클릭
                    print(f"파일 선택 버튼 클릭 및 파일 설정 중: {full_file_path}")
                    try:
                        # filechooser 이벤트 대기
                        async with page.expect_file_chooser(timeout=5000) as fc_info:
                            await modal_btn.click()
                            print("✓ 파일 선택 버튼 클릭 완료")
                        
                        file_chooser = await fc_info.value
                        print("✓ 파일 선택 다이얼로그 감지됨")
                        
                        # 파일 설정 (다이얼로그 없이 백그라운드에서 처리)
                        await file_chooser.set_files(str(full_file_path))
                        print("✓ 파일 설정 완료 (백그라운드 업로드)")
                        await asyncio.sleep(1.5)  # 파일 업로드 처리 대기
                    except Exception as e:
                        print(f"✗ filechooser 방식 실패: {e}")
                        print("기존 방식(pyautogui)으로 폴백합니다...")
                        # 폴백: 버튼 클릭 후 pyautogui 사용
                        await modal_btn.click()
            ...
            async def process_single_task(page, settings, model_name, clothes_name, current_task, total_tasks, delete_selector_1):
                ...
                # GUI 업데이트
                if gui_instance:
                    gui_instance.current_loop = current_task
                    def update_gui():
                        gui_instance.progress_label.config(text=f"진행: {current_task}/{total_tasks}", fg="blue")
                        gui_instance.log(f"[{current_task}/{total_tasks}] {model_name} + {clothes_name} 처리 중...")
                    gui_instance.root.after(0, update_gui)
                ...
                # 4단계: 이미지 생성 완료 대기
                print("\n=== 4단계: 이미지 생성 완료 대기 ===")
                completion_result = await wait_for_image_completion()
                if not completion_result or should_stop():
                    return
            ...
            # 중지 체크 함수
            def should_stop():
                if gui_instance and not gui_instance.is_running:
                    if gui_instance:
                        gui_instance.log("작업 중지 요청됨...")
                    return True
                return False
            ...
            if settings["mode"] == "auto":
                ...
                log_msg("\n" + "=" * 60)
                log_msg("지정모드 - 선택한 파일로 자동화 시작!")
                log_msg(f"모델 파일: {len(model_files)}개")
                log_msg(f"클로즈 파일: {len(clothes_files)}개")
                log_msg(f"총 작업 횟수: {total_tasks}번")
                log_msg("=" * 60)
                log_msg("\n[루프 순서] 외부 루프: 클로즈 파일들 → 내부 루프: 모델 파일들")
                log_msg("예시: 클로즈1 + 모든 모델 → 클로즈2 + 모든 모델 → ...")
                log_msg("=" * 60)
                
                # 외부 루프: clothes 파일들
                for clothes_idx, clothes_file in enumerate(clothes_files, 1):
                    if should_stop():
                        log_msg("\n작업이 중지되었습니다.")
                        return
                    
                    log_msg(f"\n{'='*60}")
                    log_msg(f"[외부 루프 {clothes_idx}/{len(clothes_files)}] 클로즈: {clothes_file.name}")
                    log_msg(f"이 클로즈와 {len(model_files)}개의 모델을 모두 조합합니다.")
                    log_msg(f"{'='*60}")
                    ...
                    for model_idx, model_file in enumerate(model_files, 1):
                        ...
                        total_work += 1
                        log_msg(f"\n{'─'*60}")
                        log_msg(f"[작업 {total_work}/{total_tasks}] 클로즈 {clothes_idx}/{len(clothes_files)} + 모델 {model_idx}/{len(model_files)}")
                        log_msg(f"조합: {clothes_file.name} + {model_file.name}")
                        log_msg(f"{'─'*60}")
                        ...
                        try:
                            ...
                            log_msg(f"\n✓ 조합 완료: {clothes_file.name} + {model_file.name}")
                            
                        except Exception as e:
                            log_msg(f"\n✗ 조합 실패: {clothes_file.name} + {model_file.name}")
                            log_msg(f"오류: {e}")
                            log_msg("다음 조합으로 계속 진행합니다...")
                            import traceback
                            traceback.print_exc()
                            continue
                    
                    log_msg(f"\n{'='*60}")
                    log_msg(f"✓ 클로즈 {clothes_idx}/{len(clothes_files)} ({clothes_file.name}) 완료!")
                    log_msg(f"  → {len(model_files)}개의 모델과 모두 조합했습니다.")
                    log_msg(f"{'='*60}")
            
            # 모든 작업 완료
            log_msg("\n" + "=" * 60)
            log_msg("모든 작업 완료!")
            log_msg(f"총 {total_work}개의 작업을 완료했습니다.")
            log_msg("=" * 60)
            print("\n브라우저를 닫지 마세요.")
